...
class Models {
  ...
  findById = async (req, res, next = undefined) => {
    ...
    if (user_data) {
      const { exclude, modelIdLabel } = require(`../../app/Models/${this.modelName}`);
      if (this.modelName !== "User") {
        where = `${where} AND ${[modelIdLabel]} = ($2)`;
        values.push(user_data._id);
      }
      if (exclude.length > 0) {
        where = `${where} AND column_name NOT IN ('${exclude.join("', '")}')`;
      }
    }
    ...
    if (_id) {
      try {
        ...
        if (next) {
          req[this.modelName.toLowerCase()] = rows;
          return next();
        }
        res.status(200).json({ [this.modelName.toLowerCase()]: rows })
      } catch (err) {
